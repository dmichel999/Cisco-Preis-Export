...
`findQuoteTotalCell` (`js/app.js`) sucht dafür nur noch einen Textanker über das gesamte Blatt: eine Zeile mit Zelltext "Quote Total". Als Quelle gilt die **rechteste plain-numerische Zelle derselben Zeile** — nicht mehr der Schnittpunkt mit einem zweiten, unabhängigen Spalten-Anker ("Special Items Total"-Kopfzeile, bis 0.23.0). Fehlt der Anker (normale Quotes ohne diesen Block), bleibt das Feature stiller No-op, komplett unabhängig von `findQuoteTable`/der Artikeltabelle.

**Warum kein Spalten-Anker mehr (0.23.0):** Manche Quotes haben zwei unabhängige Financial-Summary-Blöcke auf demselben Blatt — einen oberen mit eigener Kopfzeile ("... List Price" / "Discount %" / "Special Items Total") und einen unteren, reinen Label/Wert-Block ohne Kopfzeile, dessen "Quote Total"-Zeile ihren Betrag in einer ganz anderen Spalte trägt. Ein Spalten-Anker aus der oberen Tabelle passt dann nicht zur Wertspalte der unteren "Quote Total"-Zeile. Die Zeile kennt ihren eigenen Wert am zuverlässigsten selbst.
