...
6. **EUR-Preis berechnen** — jede Artikelzeile bekommt in der neuen Spalte (direkt nach der letzten Bestandsspalte, aktuell AF, Kopfzeile "Price EUR") eine **Excel-Formel** auf Basis von "Unit Net Price Before Credits": `=ROUND(<Quellzelle>/$AF$39,2)`, angezeigt im Euro-Zahlenformat (`#,##0.00 €`). Ändert man den Kurs in AF39 direkt in Excel, rechnet Excel alle Preise automatisch neu — kein erneuter Durchlauf durchs Tool nötig. Die neue Spalte (Kurs-Zelle, Kopfzeile, alle Datenzeilen) wird gelb hinterlegt (`#FFFF01`). Eine nicht-numerische oder fehlende Quellzelle (z. B. Ciscos `"--"`-Platzhalter für Zeilen ohne eigenen Preis) ergibt `0,00 €` statt eines Abbruchs.
7. **Subscription-Hinweis** — enthält "Pricing Term (in Months)" für eine Zeile eine Zahl (Subscription-Lizenz statt Einmalkauf), bekommt sie zusätzlich eine Zelle in einer neuen Spalte "Preishinweis" direkt hinter "Price EUR" mit dem Text "Der Einzelpreis pro X Monate = Y" (X = Pricing Term in Monaten, Y = der für diese Zeile bereits berechnete "Price EUR"-Wert). Diese Spalte fehlt komplett, wenn keine Zeile der Quote eine Subscription-Lizenz enthält.
