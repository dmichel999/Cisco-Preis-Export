...
7. **Subscription-Hinweis** — enthält die Zeile eine Spalte "Pricing Term (in Months)" mit einem Wert > 0 (Subscription-Lizenz statt Einmalkauf), bekommt sie zusätzlich eine Zelle in einer neuen Spalte "Preishinweis" mit dem Text "Der Einzelpreis pro X Monate = Y" (X = Pricing Term in Monaten, Y = der aus "Unit List Price" berechnete EUR-Preis dieser Zeile, auch wenn Y = 0,00 €). Y kommt bewusst aus einer anderen Spalte als die Haupt-"Price EUR"-Spalte (siehe architecture.md) und kann sich davon unterscheiden. Die Preishinweis-Spalte fehlt komplett, wenn keine Zeile der Quote eine Subscription-Lizenz enthält.
